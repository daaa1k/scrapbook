...
Note shell is the `/notebooks/$notebookId` three-pane experience: sources on the left, summarize/citations/ask in the center, memo on the right. The notebook title is the page `h1`. Rename is not a permanent field. Pane titles `ソース`, `要約・質問`, and `メモ` are `h2`. Layout modes by width: phone tabs below `md` (48rem), tablet source drawer from `md` to `lg`, desktop three-pane split at `lg` (64rem). The notebook route fills the viewport height. The notebook header stays fixed. Each pane scrolls on its own.
...
- `shell-layout` keeps the notebook chrome in `h-dvh` with `overflow` on the panes, not the page. Desktop columns use flexible `minmax` tracks (not fixed `280px + 320px`). Tablet shows study + memo side by side and opens sources with `ソース一覧` as a left drawer. Phone keeps the three tabs. Pane surfaces use tinted backgrounds and sticky `h2` headings, not borders alone. Notebook max width is `90rem` on this route.
