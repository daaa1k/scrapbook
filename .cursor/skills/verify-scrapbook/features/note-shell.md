...
- **Citations.** After summarize (mock or live), summary text ends with `[1]` footnote buttons (`getByRole('button', { name: '引用1' })`). Hover, focus, or click opens the excerpt panel (`aria-expanded`). There is no separate `引用` region.
