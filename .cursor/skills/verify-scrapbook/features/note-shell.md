...
- `shell-summarize` / `shell-ask` run from the center pane. In-flight jobs use Japanese labels and a spinner (`要約を準備しています`, `回答しています`). A waiting turn does not say `回答待ち…`. Failures use an alert with a next action. See [Job progress and recovery](./job-progress.md). On phone, `質問する` sits in a sticky footer inside the study scrollport so it stays reachable when the software keyboard shrinks the visual viewport.
