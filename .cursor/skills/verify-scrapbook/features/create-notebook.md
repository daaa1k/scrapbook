...
- Open `/` (header brand `Scrapbook` or nav「ノート」).
...
- On home, choose `削除` and accept the confirm that names cascade deletion.
...
- **Delete.** Return to `/`. Click `削除` for that card. Accept `window.confirm`. The card disappears.
...
- Delete always confirms. Processing jobs reject notebook delete with a Japanese error.
