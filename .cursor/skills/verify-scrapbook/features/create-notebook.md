...
- `notebook-open` opens `/notebooks/$notebookId` from the card body (the notebook title).
...
- **Open.** From `/`, click the notebook title in the card body. URL is `/notebooks/<uuid>`. There is no `開く` link.
