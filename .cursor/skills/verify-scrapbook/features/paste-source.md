...
- `paste-open` opens the source modal from home「新しいノート」or notebook「ソースを追加」.
...
- Under `本文を貼り付け`, fill `タイトル` and `本文`, optionally `URL（任意）`.
- Choose `本文を保存`.
...
- **Open form.** Run `.cursor/skills/verify-scrapbook/helpers/drive.mjs paste-source --title "Verify Paste Title" --body "Verify paste body for scrapbook."`. The helper opens `/`, clicks `新しいノート`, fills paste fields inside the dialog, clicks `本文を保存`.
